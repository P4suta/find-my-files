...
- **契約の正本は `fmf-contract` ただ一つ**(依存ゼロのleafクレート・定数/repr型/レイアウト/純バイト変換のみ・ロジック禁止)。**散文の写しを作らない**(値・オフセット・名簿をドキュメントに複製しない)。codecは `fmf-proto`。変更フローは一方向放射: fmf-contract → `FMF_BLESS=1` 再捕獲 → `just contract-gen` → 両言語テストgreen(ADR-0018)。エラーコードは**追記のみ・再番号禁止**
...
- **ビルド出力は repo直下 `build/` の単一ツリーに集約**(ADR-0021)。**出力パスの正本は `xtask/src/paths.rs`**(サブディレクトリをここに列挙しない)。**C# obj だけは `app/**/obj/` に据え置き**(移設は禁止の `Directory.Build.props` が必要)。`.gitignore` は `build/` 1行で全成果物を除外。配布バンドルは版/チャネル/commit/date を刻む(ADR-0038)
- pipe のセキュリティは `docs/SECURITY.md` が正本。**SDDL構築は fmf-service/src/security.rs の構築関数経由のみ**(直書き禁止)

## 開発環境(このマシンは chezmoi + mise 管理)

- ローカル開発ツールは `mise.toml` でピン留め。**rustup・winget等でのアドホックな導入はしない**。新ツールが要るなら `mise.toml` に追記して `mise install`
- **CI は mise 環境を再現しない**。公式 setup Action と checksum 検証付きの定評ある install Action で、各jobが使う最小ツールだけを固定導入する。ローカルとの共有pinは機械的に漂流検出する
- タスクランナーは `just`。**レシピ一覧の正本は `just`(=`just --list`)**(各レシピが説明つきでグループ表示される。**ここに写さない**)。新しい定常タスクはレシピ化し、説明を付ける
...
- ログ: エンジン=`%ProgramData%\find-my-files\logs\engine.<date>.log`(`FMF_LOG`でフィルタ)、アプリ=`%APPDATA%\find-my-files\logs\app.log`。異常調査はまずログフォルダー+F12パネル
...
- `engine/crates/fmf-contract` — 契約の正本(状態コード・opcode・wire構造体・上限・protocol/pipe/service名)。散文の代わりにここと `contract/golden/` を読む
- `docs/adr/` — 設計判断・却下判断の記録(数値根拠・再検討トリガつき)。**構造を変える前に該当ADRを読む**
- `docs/SECURITY.md` — pipe/サービスのセキュリティモデルの正本
- `docs/RESEARCH.md` — 裏取り済み外部事実(MFT/USN API仕様、出典付き)。**設計判断の前に必ず読む**
