...
- **Persistence**: `{index_dir}\{drive-letter}.fmfidx` (e.g. `c.fmfidx`), format FMFIDX05 ([ADR-0010](adr/0010-snapshot-raw-pod-no-compat.md), mtime u32 seconds per [ADR-0031](adr/0031-mtime-u32-unix-seconds.md)). temp→`MoveFileEx(REPLACE_EXISTING)`. On startup: load→verify→USN replay→live tail. Failure always falls back to a full rescan.
...
```json
{ "volumes": ["C:"], "log_level": "info", "flush_interval_secs": 300, "idle_stop_secs": 300, "gc_max_idle_days": 7, "authorized_sids": ["S-1-5-21-…"] }
```

- **On-demand lifecycle ([ADR-0027](adr/0027-on-demand-service-lifecycle.md))**: the service is `DEMAND_START` (not boot-resident). Install copies `fmf-service.exe` into the hardened data root and registers/points the GC task at that stable copy; sets the service-object DACL so the authorized user SID(s) can `start`/`stop` unelevated (start/stop/query only — never change-config/delete, which would be LPE on a LocalSystem service). The app starts it on launch (`StartThenPipe`); `serve()` self-stops after `idle_stop_secs` (default 300, `0` = stay resident) with no live connection; a daily SYSTEM Scheduled Task runs `fmf-service gc`, which uninstalls + purges when `last_use` (a machine-wide stamp the service refreshes on connect/stop) is older than `gc_max_idle_days` (default 7, `0` = disabled). Teardown is complete: `uninstall --purge-data` (the app's "Remove" + "also delete index/logs") removes the service key, the Scheduled Task, and the whole data dir (incl. the stable exe) at once; plain `uninstall` also drops the exe + task, keeping only the user's index/logs/`service.json`; the GC defers only the running exe + its dir to the next reboot. Nothing is written outside the SCM key, `%ProgramData%\find-my-files`, and the task.
...
- **Log format (both languages, [ADR-0037](adr/0037-logfmt-diagnostics-and-correlation.md))**: one **logfmt** line `ts level area [fields…] msg="…" [err="…"]` — human-readable *and* grep/awk-parseable. `ts` is RFC3339 + local offset; values are emitted bare unless they contain a space/`=`/`"`/control char, in which case they are quoted with `\r`/`\n`/`\uXXXX` escaping. That quoting is the **log-injection defence**: a query or NTFS name carrying CR/LF can never forge a second line. Values are capped at 1 KiB. The engine logfmt event formatter lives in `fmf-core::diag` (`LogfmtFormat`); the app's in `LogfmtFormatter` behind Serilog (the only logging dependency).
- **Logs**: engine=`%ProgramData%\find-my-files\logs\engine.<date>.log` (daily rotation, `max_log_files` generations — 14 for the service, 7 for FFI/CLI; filter via the `FMF_LOG` env var), app=`…\logs\app.log` (5 generations, rolled at 5MB; `<exe>\data\logs` portable, else `%APPDATA%\find-my-files\logs`). The app reads `FMF_LOG` too (shared spelling) for its initial level
- **Cross-process correlation (contract-unchanged)**: one user query ties `app.log` to `engine.log` via the existing wire/handle ids — **no contract/golden change**. On the **pipe** path the engine groups a request's lines under `qid` = the frame `request_id` (already client-generated and echoed); on **both** paths the per-query "query served" line carries `rid` = the result handle (resultId on the pipe, the boxed handle's address on the in-process FFI path), which the UI logs too — `rid` is the universal join key. The query *text* is never logged (only its length) — filenames/queries are the sensitive asset (redaction)
