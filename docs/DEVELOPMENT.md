...
- **Procedural build/publish/verification logic lives in the `xtask/` crate** (the cargo-xtask pattern); `just` is a thin wrapper. `xtask` is its own workspace at the repo root — never a member of the engine workspace. Pure logic in `xtask` gets unit tests; never inline PowerShell back into a recipe. **Versioning is not xtask's job** — the version, CHANGELOG, and tag are owned by release-please ([ADR-0035](adr/0035-automated-versioning-with-release-please-and-build-channels.md)); `xtask version` only formats the channel-aware build identity.
- **Versioning is automated from Conventional Commits.** release-please keeps a Release PR open; merging it bumps the version + CHANGELOG and cuts the `vX.Y.Z` tag (no hand-picking a number). Builds are stamped `X.Y.Z-dev+g<sha>` (local) / `-nightly.<date>+g<sha>` / clean `X.Y.Z` (stable). See [RELEASING.md](RELEASING.md).
- Git hooks are managed by **lefthook** (`just setup` installs them). commit-msg: Conventional Commits (`committed`). pre-commit: typos + rustfmt + taplo. pre-push: clippy + test + test-app (+ the xtask checks when `xtask/` changed). **Never bypass a hook** (`git push --no-verify` is forbidden) — the hooks are the quality gate.
