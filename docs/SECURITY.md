...
- [ ] A pipe connection from another user (non-authorized SID) is rejected — *not performed 2026-07-01: no second user account available (the SDDL build is pinned by unit tests; a live cross-user check still needs a second SID)*
- [ ] A remote connection to `\\<host>\pipe\fmf-engine-v2` is rejected — *not performed 2026-07-01: no second machine/network host available*
...
- [ ] After OTS elevation (elevated with a different admin account), the everyday user can still connect to the pipe non-elevated (`--owner-sid` propagation) — *not performed 2026-07-01: no second admin account available*
- [ ] "Re-register" to a running service → restart reflects `authorized_sids`, and a previously-rejected user can connect (`pipe client token rejected` stops) — *not performed 2026-07-01: no second user account available*
- [x] Leftovers after `fmf-service uninstall` match the guidance / are removed by `--purge-data` (2026-07-01: elevated on-machine — `uninstall` deregistered the service (`sc query` → 1060) and `--purge-data` removed `%ProgramData%\find-my-files` entirely; confirmed the dir no longer exists)
- [x] (ADR-0027) `sc qc fmf-engine` shows `DEMAND_START` (not AUTO_START); `sc sdshow fmf-engine` grants the user SID start/stop/query only — no `DC`/`SD`/`WD`/`WO` (2026-07-01: `START_TYPE: 3 DEMAND_START`; install-user ACE `CCLCSWRPWPLORC` = query-config/query-status/enum/`RP`(start)/`WP`(stop)/interrogate/read — none of `DC`/`SD`/`WD`/`WO`; only `SY`+`BA` hold full control)
- [x] (ADR-0027) `%ProgramData%\find-my-files\fmf-service.exe` is **not** writable by a standard user (icacls: SYSTEM + Administrators only); a non-authorized user cannot start/stop the service (2026-07-01: `icacls` shows `SYSTEM:(F)` + `Administrators:(F)` only, no `BUILTIN\Users`; the cross-user start/stop denial follows structurally from the `sdshow` ACE above — a live second-user test still pending)
- [ ] (ADR-0027) App launch starts the stopped service **without a UAC prompt**; closing the app self-stops it after `idle_stop_secs`; `fmf-service gc` with an aged `last_use` removes service + Scheduled Task + data — *no-UAC app-launch + idle-stop pending (MANUAL_SMOKE.md, non-elevated standard user). The GC **Scheduled Task** registration was found broken on ja-JP and fixed 2026-07-01 (see Implementation record); re-install now registers it and uninstall removes it cleanly.*
...
- **Elevated on-machine verification (2026-07-01, ja-JP)**: re-confirmed the `index/` and `fmf-service.exe` DACLs are SYSTEM + Administrators only (`icacls`), the service is `DEMAND_START` with a start/stop/query-only user ACE (no `DC`/`SD`/`WD`/`WO`), and `uninstall` + `--purge-data` leaves no service registration and no data root. **Found and fixed a bug**: the daily GC **Scheduled Task** failed to register on a Japanese-locale install — `register_gc_task` wrote the task XML as UTF-8 while `schtasks /Create /XML` begins reading it as UTF-16 and aborts with "Cannot switch the encoding" at the declaration `(1,40)` (tolerated on en-US, fatal on ja-JP). Fix: serialize the definition as **UTF-16LE + BOM** under an `encoding="UTF-16"` declaration (`fmf-service::lifecycle::gc_task_xml`, unit-tested for the BOM + declaration); re-install now prints `registered daily GC task` and uninstall deletes it cleanly. The service still functioned without the task (DEMAND_START + idle self-stop); only the weekly self-removal was affected.
- **Runtime sign-off (remaining, not yet done)**: the items still marked `[ ]` above require another user's token, a remote host, OTS elevation with a second admin account, or the non-elevated UAC/single-window smoke (`MANUAL_SMOKE.md`). Run them in an elevated multi-user/network environment before release, with the date recorded.
