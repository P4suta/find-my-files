...
Date: 2026-06-17 / Status: Superseded by [ADR-0024](0024-non-elevated-scope-index.md) on 2026-07-26, which removed scope mode itself; the exclude mechanism described here went with it.

Kept rather than deleted because a decision record is only useful if the
rejected and retired options stay readable next to the surviving one — the
same reason [ADR-0028](0028-msix-packaging-hybrid.md) stays as `Rejected`.
It was removed from the tree outright in `0f3d873`, which also left a hole
in the numbering that reads as an accident rather than a decision.
