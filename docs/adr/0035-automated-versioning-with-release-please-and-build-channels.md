...
- **First activation must be verified**: the first Release PR should show `engine/Cargo.toml` + `engine/Cargo.lock` + the csproj all bumped and `engine/CHANGELOG.md` written. (The Rust strategy was confirmed to handle the virtual `[workspace.package]` manifest correctly on the first dry run; the only config gotcha found was that `changelog-path` rejects `..`, hence the changelog lives under `engine/`.) If a future release-please version regresses workspace handling, the fallback is a `toml` extra-file (`$.workspace.package.version`) plus a lock-refresh step. (Documented in `docs/RELEASING.md`.)
