...
2. **Version stays declared in the files; the bot edits them (not git-derived).** The package `engine` uses `release-type: "simple"` with `extra-files`: a `toml` updater sets `engine/Cargo.toml` `$.workspace.package.version`, and a `generic` updater (keyed on an `x-release-please-version` annotation) sets the csproj `<Version>`. We do **not** use release-please's `rust` release-type: it can't write a workspace-*inherited* version (`version.workspace = true`) and fails with "value at path package.version is not tagged" (googleapis/release-please#2478, #1170). Because the `toml` updater bumps `Cargo.toml` but not `engine/Cargo.lock` (and CI is `--locked`), `release-please.yml` runs `cargo update --workspace` on the Release PR branch to sync the lock (no compile; re-runs on every PR rebuild so it self-heals). The CHANGELOG lives at `engine/CHANGELOG.md` (not the repo root): the package is `engine` and release-please's `changelog-path` is package-relative and rejects `..`. The manifest keeps the version present and reproducible (tarball/`.git`-less builds, `cargo metadata`, debuggability) — the "shackle" was the *human driver*, already removed by (1), not the stored number.
...
- **First activation must be verified**: the first Release PR should show `engine/Cargo.toml` `[workspace.package] version` bumped (by the `toml` updater), `engine/Cargo.lock` synced (by the `cargo update --workspace` step, as a follow-up commit on the PR branch), the csproj `<Version>` bumped, and `engine/CHANGELOG.md` written. Two real gotchas were hit and fixed during bring-up: release-please's `rust` release-type **cannot** write workspace-inherited versions (→ `simple` + `toml` updater + the lock-sync step), and `changelog-path` **rejects `..`** (→ the changelog lives under `engine/`).
