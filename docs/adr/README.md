...
- [0029](0029-ci-signing-cka-pipeline.md) — CI signing pipeline: standard `signtool` via eSigner CKA (replaces the Java `batch_sign` Action), `build`→`sign`→`publish` job split with secrets in an approval-gated `release` environment, checksum-pinned CKA; supersedes ADR-0020's signing *mechanism* (provider/cert unchanged). Azure Trusted Signing (individual onboarding paused) and SignPath migration rejected
